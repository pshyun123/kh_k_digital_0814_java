...
public class CoffeeMenuRead {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        // 객체에서 읽을 수 있게 byte -> 객체화
        ObjectInputStream ois = new ObjectInputStream(fis);
        // 패키지 넘어와서 땡겨옴(패키지 '커피메뉴만들기'에서 가져옴)
        // 원래 Map<String, MenuInfo> map = new HashMap<>(); 였음 아래에 Map<String, MenuInfo> 추가
        Map<String, MenuInfo>map = (Map<String, MenuInfo>) ois.readObject(); //ois.readObject(); 형변환 자동으로
        for(String key : map.keySet()) { //key 값으로 얻어옴
